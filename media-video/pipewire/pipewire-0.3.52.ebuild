...
SRC_URI="
	https://gitlab.freedesktop.org/pipewire/pipewire/-/archive/0.3.52/pipewire-0.3.52.tar.gz -> pipewire-pipewire-0.3.52-31bf63105749893e5e2c6ae7784e7f27aa0345ad.tar.gz
...
post_src_unpack() {
	mv "${WORKDIR}"/media-session-0.4.1 "${S}"/subprojects/media-session
}
...
src_configure() {
	local emesonargs=(
		...
		-Dsession-managers="['media-session']" # All available session managers are now their own projects, so there's nothing to build
